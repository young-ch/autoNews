...
def clean_html_output(text: str) -> str:
    ...
    # 만약 토큰 한도로 인해 문자열 끝이 태그 중간에서 잘린 경우 (예: '<th style="... text-align')
    # 불완전한 여는 태그 조각을 제거하여 속성 문법 오류 방지
    text = re.sub(r'<[^>]*$', '', text)
...
4. 3대 심층 논리 분석 섹션 (<h2> 태그 활용 - 각 섹션마다 핵심 1~2문단씩 밀도 높게 서술, 전체 분량 1,200~1,500자 내외):
   - <h2 style="color:#0f172a; border-bottom:2px solid #e2e8f0; padding-bottom:8px; margin-top:35px;">1. 글로벌 매크로 지표 분석: 미국채 금리 & 원유가 국장에 미칠 유동성 파급력</h2>
     * 단순히 금리 몇 %, 유가 몇 달러라는 숫자 나열을 절대 지양하고, 해당 지표의 변동이 오늘 환율과 외국인/기관의 국장 순매수 여력에 미칠 구체적 영향을 인과관계 중심으로 1~2문단(200자 내외) 논리 분석.
   ...
   - <h2 style="color:#0f172a; border-bottom:2px solid #e2e8f0; padding-bottom:8px; margin-top:35px;">3. 오늘 한국 시장(국장) 실전 투자 전략 & 시초가 관전 포인트</h2>
     * 오늘 코스피/코스닥의 예상 개장 시초가 분위기, 갭상승 시 추격매수 자제 구간, 눌림목에서 자금 유입이 기대되는 수혜 테마 등 실전 투자자의 독창적인 관점(View)을 1~2문단으로 상세 서술.
...
5. 📅 [오늘의 주요 글로벌 경제 캘린더 & 변동성 대비]:
   ...
   - 표 하단에 해당 지표 발표 시 장중 변동성에 대비한 리스크 관리 팁 1문단 추가.
...
7. 출력 규칙:
   ...
   - 글이 중간에 끊기지 않고 1번부터 끝까지 100% 완결되도록 모든 태그를 끝까지 완벽하게 닫을 것.
"""
...
따라서 제공된 정량 데이터(지표, 섹터, 뉴스)를 바탕으로, 단순 나열을 완전히 배제하고 "자금 흐름의 논리적 인과관계와 오늘 국장 시초가 대응 시나리오"를 1,200~1,500자 내외의 전문적인 리포트 형식으로 완성도 높게 작성해야 해.
