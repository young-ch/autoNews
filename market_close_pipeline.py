...
개인 투자자들이 퇴근길에 읽고 오늘 시장의 맥락과 오늘 밤/내일장을 완벽하게 대비할 수 있는 최고급 장마감 시황 브리핑 리포트를 작성해 줘.
...
5. 📅 [오늘 밤 & 내일 글로벌 핵심 경제 캘린더 프리뷰] (★ 매우 중요):
   ...
   - 제공된 [오늘의 경제 캘린더] 데이터를 바탕으로, 세련된 HTML <table>을 생성할 것:
     (컬럼: 발표 시간 / 국가 / 경제 지표명 / 중요도 / 시장 예상치 / 직전치 / 시장 관전 포인트)
     - 중요도 '높음'인 항목은 빨간색 뱃지/볼드 강조 스타일 적용
   - 테이블 아래에 [애널리스트 분석 코멘트]:
     - 오늘 밤 발표될 주요 경제 지표(미국 고용, 실업률, CPI 등) 결과가 달러 환율, 미 국채금리, 한국 야간선물 및 내일 코스피 시초가에 미칠 시나리오별 파급 효과(예상치 상회 vs 하회)를 명쾌하게 분석할 것.
